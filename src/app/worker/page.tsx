...
export default function DashboardPage() {
  ...
  // Mapa de workers: clave = parte antes del @ (slug)
  ...
  data.forEach((item) => {
    ...
    // Buscar en la tabla workers por email exacto (ignorando mayúsculas)
    ...
    // 🔥 CALCULAR PAGO AL TRABAJADOR – SOLO DESDE LA TABLA workers
    let costPEN = 5 // valor por defecto seguro (solo si no hay workerData)
    if (workerData) {
      let pagoUnas = Number(workerData.pago_unas)
      let pagoSeguros = Number(workerData.pago_seguros)
      // Si no es número válido, usar 5 como fallback (pero respetar 0 si es 0)
      if (isNaN(pagoUnas)) pagoUnas = 5
      if (isNaN(pagoSeguros)) pagoSeguros = 5
      costPEN = service === 'seguros' ? pagoSeguros : pagoUnas
    } else {
      // Si el trabajador no existe en la tabla workers, por defecto 5
      // (esto no debería ocurrir si todos están registrados)
      costPEN = 5
    }
      ...
      <main className="min-h-screen bg-[#0a0a0f] text-white p-6 md:p-10">
        ...
        <div className="mb-12">
          ...
          <h1 className="text-5xl md:text-6xl font-black tracking-tight">
            <span className="bg-gradient-to-br from-white via-emerald-200 to-emerald-500 bg-clip-text text-transparent">Easy Money</span>
          </h1>
        ...
        <div className="grid gap-5 md:grid-cols-3 mb-12">
          ...
          <div className="relative overflow-hidden bg-gradient-to-br from-slate-900 to-slate-950 border border-red-500/20 p-7 rounded-2xl group hover:border-red-500/50 transition-all duration-300">
            ...
            <div className="mt-4 h-1 w-full bg-slate-800 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-red-700 to-red-400 rounded-full" style={{ width: `${totalGenerated > 0 ? (totalPaid / totalGenerated) * 100 : 0}%` }} />
            </div>
          </div>
          ...
          <div className="relative overflow-hidden bg-gradient-to-br from-slate-900 to-slate-950 border border-cyan-500/20 p-7 rounded-2xl group hover:border-cyan-500/50 transition-all duration-300">
            ...
            <div className="mt-4 h-1 w-full bg-slate-800 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-cyan-700 to-cyan-400 rounded-full" style={{ width: `${totalGenerated > 0 ? (totalProfit / totalGenerated) * 100 : 0}%` }} />
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-8 md:grid-cols-2 mb-12">
          ...
          <div className="relative overflow-hidden bg-gradient-to-br from-slate-900 to-[#0d0d14] border border-slate-800 p-8 rounded-2xl hover:border-emerald-500/30 transition-all duration-300">
            ...
            <div className="relative z-10 flex items-center justify-between mb-8">
              <h3 className="text-3xl font-black tracking-tight flex items-center gap-3"><span className="text-4xl">💅</span><span>Uñas</span></h3>
              <button onClick={() => copyToClipboard(formatListForCopy(unasReviews), 'unas')} className={`px-4 py-2 border rounded-lg text-sm font-semibold transition-all duration-300 ${copiedButton === 'unas' ? 'bg-emerald-500/30 border-emerald-500/50 text-emerald-300 scale-105' : 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/20 hover:border-emerald-500/50'}`}>
              ...
              {unasReviews.length === 0 ? <p className="text-slate-500 text-sm">No hay registros aún</p> : (
                <div className="space-y-2">
                  {unasReviews.map((review, idx) => {
                    ...
                    return (
                      <div key={review.id} className="flex items-center justify-between bg-black border border-slate-800 rounded-xl px-5 py-3 hover:border-emerald-500/40 transition">
                        <div className="flex items-center gap-4"><span className="text-slate-500 font-mono text-sm w-8">{idx + 1}.</span><span className="font-medium tracking-wide text-white">{review.client_name}</span></div>
                        <span className="text-slate-600 text-xs tracking-widest">({workerName})</span>
                      </div>
                    )
                  })}
                </div>
          ...
          <div className="relative overflow-hidden bg-gradient-to-br from-slate-900 to-[#0d0d14] border border-slate-800 p-8 rounded-2xl hover:border-emerald-500/30 transition-all duration-300">
            ...
            <div className="relative z-10 flex items-center justify-between mb-8">
              <h3 className="text-3xl font-black tracking-tight flex items-center gap-3"><span className="text-4xl">🛡️</span><span>Seguros</span></h3>
              <button onClick={() => copyToClipboard(formatListForCopy(segurosReviews), 'seguros')} className={`px-4 py-2 border rounded-lg text-sm font-semibold transition-all duration-300 ${copiedButton === 'seguros' ? 'bg-emerald-500/30 border-emerald-500/50 text-emerald-300 scale-105' : 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/20 hover:border-emerald-500/50'}`}>
        ...
        <div className="grid gap-5 md:grid-cols-3 mb-12">
          {workersArray.map((worker: any) => (
            <div key={worker.name} className="bg-gradient-to-br from-slate-900 to-[#0d0d14] border border-slate-800 p-7 rounded-2xl hover:border-emerald-500/30 transition-all duration-300">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div><h2 className="text-xl font-bold capitalize text-white tracking-tight">{worker.name}</h2><p className="text-slate-600 text-xs">{worker.seguros + worker.unas} servicios totales</p></div>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center py-2 px-3 rounded-lg bg-emerald-500/5 border border-emerald-500/10">
                  ...
                  <div className="text-right"><p className="text-emerald-400 font-bold text-sm">S/ {worker.generated.toFixed(2)}</p><p className="text-slate-600 text-xs font-mono">${penToUsd(worker.generated)}</p></div>
                </div>
                <div className="flex justify-between items-center py-2 px-3 rounded-lg bg-red-500/5 border border-red-500/10">
                  ...
                  <div className="text-right"><p className="text-red-400 font-bold text-sm">S/ {worker.paid.toFixed(2)}</p><p className="text-slate-600 text-xs font-mono">${penToUsd(worker.paid)}</p></div>
                </div>
                <div className="flex justify-between items-center py-2.5 px-3 rounded-xl bg-cyan-500/5 border border-cyan-500/20">
                  ...
                  <div className="text-right"><p className="text-cyan-400 font-black text-base">S/ {worker.profit.toFixed(2)}</p><p className="text-slate-500 text-xs font-mono">${penToUsd(worker.profit)}</p></div>
                </div>
              </div>
              <div className="mt-5">
                <div className="flex justify-between text-xs text-slate-600 mb-1.5"><span>Margen</span><span>{worker.generated > 0 ? ((worker.profit / worker.generated) * 100).toFixed(0) : 0}%</span></div>
                <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden"><div className="h-full bg-gradient-to-r from-cyan-600 to-emerald-400 rounded-full transition-all duration-700" style={{ width: `${worker.generated > 0 ? Math.max(0, (worker.profit / worker.generated) * 100) : 0}%` }} /></div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-12">
          <button onClick={() => setShowResetModal(true)} disabled={loading} className="flex-1 px-6 py-4 bg-red-500/10 border border-red-500/30 rounded-xl text-red-400 font-bold text-base hover:bg-red-500/20 hover:border-red-500/50 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 uppercase tracking-wide">🗑️ Reset Semanal</button>
          <button onClick={() => setShowNewWorkerModal(true)} disabled={loading} className="flex-1 px-6 py-4 bg-emerald-500/10 border border-emerald-500/30 rounded-xl text-emerald-400 font-bold text-base hover:bg-emerald-500/20 hover:border-emerald-500/50 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 uppercase tracking-wide">➕ Añadir Worker</button>
        </div>

        {/* MESSAGE */}
        {message && <div className={`mb-8 rounded-xl border px-6 py-4 text-center text-sm ${messageStyles[messageType]}`}>{message}</div>}
        ...
        {showNewWorkerModal && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-40 flex items-center justify-center p-4">
            <div className="bg-gradient-to-br from-slate-900 to-[#0d0d14] border border-slate-800 rounded-2xl p-8 w-full max-w-md shadow-2xl">
              <h3 className="text-2xl font-black mb-6 text-white flex items-center gap-2"><span>➕</span><span>Nuevo Worker</span></h3>
              <div className="space-y-4 mb-6">
                <div><label className="block text-slate-400 text-sm font-semibold mb-2">Nombre</label><input type="text" placeholder="Ej: Carlos" value={newWorkerForm.name} onChange={(e) => setNewWorkerForm({...newWorkerForm, name: e.target.value})} className="w-full bg-black border border-slate-800 rounded-xl px-4 py-3 text-white placeholder-slate-600 focus:border-emerald-500 outline-none transition" /></div>
                <div><label className="block text-slate-400 text-sm font-semibold mb-2">Email</label><input type="email" placeholder="ej: carlos@easymoney.com" value={newWorkerForm.email} onChange={(e) => setNewWorkerForm({...newWorkerForm, email: e.target.value})} className="w-full bg-black border border-slate-800 rounded-xl px-4 py-3 text-white placeholder-slate-600 focus:border-emerald-500 outline-none transition" /></div>
                <div><label className="block text-slate-400 text-sm font-semibold mb-2">Pago Uñas (soles)</label><input type="number" step="0.01" placeholder="Ej: 5" value={newWorkerForm.pago_unas} onChange={(e) => setNewWorkerForm({...newWorkerForm, pago_unas: e.target.value})} className="w-full bg-black border border-slate-800 rounded-xl px-4 py-3 text-white placeholder-slate-600 focus:border-emerald-500 outline-none transition" /></div>
                <div><label className="block text-slate-400 text-sm font-semibold mb-2">Pago Seguros (soles)</label><input type="number" step="0.01" placeholder="Ej: 5" value={newWorkerForm.pago_seguros} onChange={(e) => setNewWorkerForm({...newWorkerForm, pago_seguros: e.target.value})} className="w-full bg-black border border-slate-800 rounded-xl px-4 py-3 text-white placeholder-slate-600 focus:border-emerald-500 outline-none transition" /></div>
              </div>
              <div className="flex gap-3">
                ...
                <button onClick={handleNewWorker} disabled={loading} className="flex-1 px-4 py-3 bg-emerald-500 rounded-xl text-black font-bold hover:bg-emerald-400 transition disabled:opacity-50">Guardar</button>
              </div>
            </div>
          </div>
        )}
        ...
        {showResetModal && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-40 flex items-center justify-center p-4">
            <div className="bg-gradient-to-br from-slate-900 to-[#0d0d14] border border-slate-800 rounded-2xl p-8 w-full max-w-md shadow-2xl">
              ...
              <p className="text-slate-300 mb-6">¿Estás seguro de que quieres eliminar todas las reseñas? Esta acción no se puede deshacer.</p>
              <div className="flex gap-3">
                <button onClick={() => setShowResetModal(false)} className="flex-1 px-4 py-3 border border-slate-700 rounded-xl text-slate-400 font-semibold hover:border-slate-600 hover:bg-slate-800/50 transition">Cancelar</button>
                <button onClick={handleConfirmReset} disabled={loading} className="flex-1 px-4 py-3 bg-red-500 rounded-xl text-white font-bold hover:bg-red-400 transition disabled:opacity-50">Eliminar todo</button>
              </div>
            </div>
          </div>
        )}
